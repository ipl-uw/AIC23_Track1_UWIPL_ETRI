...
def image_demo(predictor, vis_folder, current_time, args):
    ...
    root_path = osp.join(args.root_path,'data')
    input = osp.join(root_path,'test','S001')
    output = osp.join(root_path,'test_det','S001.txt')

    cameras = sorted(os.listdir(input))
    results = []
    scale = min(800/1080,1440/1920)
    for cam in cameras:
        imgs = sorted(os.listdir(osp.join(input,cam,'img')))

        timer = Timer()

        for frame_id, img_path in enumerate(imgs):
            img_path = osp.join(input,cam,'img',img_path)
